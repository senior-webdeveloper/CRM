...
async function getPagedReferenceValue(req, res) {
  try {
    ...
    const referenceValues = await sReferenceValue.getPagedReferenceValue(
      start,
      limit
    );
...
async function getExamValuebyExamId(req, res) {
  try {
    let id = req.params.id;
    let appointmentId = req.query.appointmentId;
    const referenceValue = await sReferenceValue.getExamValuebyExamId(
      id,
      appointmentId
    );
    // res.json(referenceValue);
    makeResponseOk(res, { data: referenceValue }, "referenceValue/all");
  } catch (err) {
    makeResponseException(res, err);
  }
}
